...
class Meteor(pg.sprite.Sprite):
    def __init__(self):
        pg.sprite.Sprite.__init__(self)
        choices = ['tiny','small','med','big']
        self.rd = random.randint(0,3)
        self.health = self.rd + 1
        self.image = pg.image.load(path.join(img_dir, f"meteor_{choices[self.rd]}.png")).convert()
        self.image.set_colorkey(black)
        self.rect = self.image.get_rect()
...
while True:
    ...
    hits = pg.sprite.groupcollide(bullets,meteors,True,False)
    for i in hits:
        hits[i][0].health -=1
        if hits[i][0].health <=0:
            hits[i][0].kill()
            score += 100 + hits[i][0].rd * 100
            for _ in range(1  + score //10000):
                newmeteor()
    #################################################################################################
    ...
    font = pg.font.Font(font_name, 60)  # กำหนดขนาด font
    text_surface = font.render(f"Score : {score}", True, white)  # กำหนด Text และ สี
